...
@Service
public class SewerageTaxNumberGenerator {
    private static final String DHSC_NUMBER_SEQ_PREFIX = "SEQ_DHSC_NUMBER";
    ...
    @Transactional
    public String generateDhscNumber() {
        try {
            final String sequenceName = DHSC_NUMBER_SEQ_PREFIX;
            ...
            return String.format("%s%06d", sewerageTaxUtils.getCityCode(), sequenceNumber);
        } catch (final SQLException e) {
            throw new ApplicationRuntimeException("Error occurred while generating Dhsc Number", e);
        }
    }
