 	...
 	<div class="col-sm-3 add-margin check-text text-center">
		<c:choose>
			<c:when test="${docs.isMandatory}">
				<input type="checkbox" checked disabled required="required">&nbsp;<c:out value="${docs.description}" />  
			</c:when>
			<c:otherwise>
				<input type="checkbox" disabled>&nbsp;<c:out value="${docs.description}"/> 
			</c:otherwise>		
		</c:choose> 
		<form:hidden id="appDetailsDocument${status.index}documentTypeMaster.id" path="appDetailsDocument[${status.index}].documentTypeMaster.id" value="${docs.id}" /> 
		<form:hidden id="appDetailsDocument${status.index}documentTypeMaster" path="appDetailsDocument[${status.index}].documentTypeMaster.isMandatory" value="${docs.isMandatory}" /> 
		<form:hidden id="appDetailsDocument${status.index}documentTypeMaster.description" path="appDetailsDocument[${status.index}].documentTypeMaster.description" value="${docs.description}" /> 
	</div>
